/* =========================================================
   MESA 360 v0.5 · MADUREZ PRE-BACKEND
...
function ensureTicketModel(t){
  if(!t.messages){
    t.messages=(t.timeline||[]).map(([text,at])=>({kind:'system',visibility:'public',author:'MESA 360',text,at}));
  ...
  t.related=t.related||[];
  t.audit=t.audit||[{action:'Solicitud registrada',actor:'MESA 360',at:t.created}];
  return t;
}
...
function addTicketCommentV5(id){
  ...
  const author=currentRole==='requester'?'Juan Pérez':personById(t.assignee)?.name||'Mesa 360 · Gestor';
...
document.addEventListener('click',e=>{
  ...
  if(e.target.closest('[data-reopen-ticket]')&&ticketDrawerState.id){const t=tickets.find(x=>x.id===ticketDrawerState.id);ensureTicketModel(t);t.status='En gestión';t.requesterActionRequired=false;t.messages.push({kind:'system',visibility:'public',author:'MESA 360',text:'El funcionario reabrió la solicitud porque requiere atención adicional.',at:'Ahora'});saveTickets();renderTicketDrawerV5();showToast('Solicitud reabierta','El caso volvió a la cola del equipo gestor.');return;}
},true);
...
setView=function(view){
  ...
  const names={home:'Inicio','new-request':'Nueva solicitud','my-tickets':'Mis solicitudes',notifications:'Actualizaciones',knowledge:'Centro de conocimiento',status:'Estado de servicios',calendar:'Agenda y disponibilidad',ops:'Centro de operaciones',continuity:'Incidentes, problemas y cambios',team:'Equipo y capacidad',assets:'Activos y CMDB',catalog:'Catálogo de servicios',reports:'Indicadores',workflows:'Flujos y automatización',admin:'Configuración'};$('#breadcrumb').textContent=`Mesa 360 / ${names[view]||'Inicio'}`;renderView(view);if(innerWidth<860)$('#sidebar').classList.remove('open');
};
