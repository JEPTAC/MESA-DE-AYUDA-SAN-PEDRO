/* =========================================================
   MESA 360 v0.6 · ADMIN STUDIO / ESM CONTROL PLANE
...
const v06RoleSeed = [
  ...
  {id:'admin',name:'Administrador MESA 360',desc:'Gobierno integral de plataforma y configuración.',mappedRole:'admin'}
];
...
function baseControlPlane(){
  const serviceRecords={};services.forEach(s=>{const snap=makeServiceSnapshot(s);serviceRecords[s.id]={version:1,status:'published',published:snap,draft:clone(snap),updated:'20 Ago 2026 · 8:18 a. m.',owner:'Administrador MESA 360'};});
  ...
  return {version:V06_VERSION,departments:clone(v06DepartmentsSeed),services:serviceRecords,roles:clone(v06RoleSeed),roleMatrix:defaultRoleMatrix(),notificationRules:clone(notificationSeed),templates:clone(responseTemplateSeed),workflows:flows,audit:[
    {id:'AUD-001',at:'20 Ago 2026 · 8:18',actor:'Administrador MESA 360',action:'Control plane v0.6 inicializado',object:'Plataforma',detail:'Se habilitó administración no-code en modo pre-Supabase.',severity:'info'},
...
function auditV6(action,object,detail,severity='info',actor='Administrador MESA 360'){
...
function renderTemplateStudio(root){
  ...
  root.innerHTML=`<div class="template-studio"><aside class="template-list"><div class="studio-pane-head"><div><strong>Respuestas rápidas</strong><span>${list.length} plantillas</span></div><button class="studio-icon-btn" data-template-add>＋</button></div>${list.map(t=>`<button class="template-item ${t.id===sel?.id?'active':''}" data-template="${t.id}"><span>${t.category==='TIC'?'⌁':t.category==='Comunicaciones'?'✦':'◇'}</span><div><strong>${safe(t.name)}</strong><small>${safe(t.category)} · ${safe(t.scope)}</small></div></button>`).join('')}</aside>${sel?`<section class="template-editor"><div class="studio-builder-head"><div><span class="eyebrow">MACROS · RESPUESTAS CONSISTENTES</span><h2>${safe(sel.name)}</h2><p>Las plantillas aceleran la operación sin sacrificar claridad ni trazabilidad.</p></div><button class="btn btn-primary" data-template-save>✓ Guardar</button></div><div class="template-form"><label>Nombre<input data-template-prop="name" value="${safe(sel.name)}"></label><div class="template-grid"><label>Categoría<input data-template-prop="category" value="${safe(sel.category)}"></label><label>Disponible para<input data-template-prop="scope" value="${safe(sel.scope)}"></label></div><label>Contenido<textarea data-template-prop="body" rows="10">${safe(sel.body)}</textarea></label><div class="variable-chips big"><span>Insertar variable</span>${['{{requester.name}}','{{ticket.id}}','{{ticket.title}}','{{assignee.name}}','{{schedule.date}}','{{schedule.start}}','{{schedule.end}}'].map(v=>`<button data-template-variable="${safe(v)}">${safe(v)}</button>`).join('')}</div></div><div class="template-preview"><span>VISTA PREVIA</span><div class="message-bubble"><strong>MESA 360 · Gestor</strong><p>${safe(templateExample(sel.body))}</p><small>Visible al funcionario · ahora</small></div></div></section>`:''}</div>`;
}
...
setRole=function(role){
  ...
  showToast('Perfil simulado',`Ahora estás viendo MESA 360 como ${r.name.toLowerCase()}.`);
};
...
addTicketCommentV5=function(id){
  ...
  if(text)auditV6(visibility==='internal'?'Nota interna registrada':'Respuesta al funcionario publicada',id,text.slice(0,120),visibility==='internal'?'warning':'info',currentRole==='admin'?'Administrador MESA 360':'Gestor de servicio');
};
...
document.addEventListener('click',e=>{
  if(e.target.closest('[data-insert-template]')){
    ...
    const assignee=personById(ticket.assignee)?.name||ticket.assignmentGroup||'Mesa 360';let body=t.body.replaceAll('{{requester.name}}',ticket.requester||'Funcionario').replaceAll('{{ticket.id}}',ticket.id).replaceAll('{{ticket.title}}',ticket.title).replaceAll('{{assignee.name}}',assignee).replaceAll('{{schedule.date}}',ticket.details?.scheduledDate||'fecha programada').replaceAll('{{schedule.start}}',ticket.details?.scheduledStart||'hora inicial').replaceAll('{{schedule.end}}',ticket.details?.scheduledEnd||'hora final');
...
function createNewRole(){const name=$('#newRoleName')?.value.trim();if(!name)return showToast('Falta el nombre','Escribe un nombre para el perfil.');const id=`role-${Date.now()}`,mappedRole=$('#newRoleMapped').value,desc=$('#newRoleDesc').value.trim()||'Perfil configurable de MESA 360.';controlPlane.roles.push({id,name,desc,mappedRole});controlPlane.roleMatrix[id]={};adminStudio.role=id;saveControlPlane();setupRoleSwitcher();auditV6('Rol creado',name,`Experiencia base: ${mappedRole}.`,'warning');closeStudioDialog();renderAdmin();}
